...
export default function HomePage() {
  ...
  const CATEGORIES = [
    { id: 1, label: t("cat_cars"), icon: CarIcon, color: "bg-blue-50 text-blue-600" },
    { id: 2, label: t("cat_trucks"), icon: Truck, color: "bg-indigo-50 text-indigo-600" },
    { id: 3, label: t("cat_machinery"), icon: Construction, color: "bg-amber-50 text-amber-600" },
    { id: 4, label: t("cat_equipment"), icon: Wrench, color: "bg-emerald-50 text-emerald-600" },
    { id: 5, label: t("cat_water"), icon: Ship, color: "bg-cyan-50 text-cyan-600" },
    { id: 6, label: t("cat_more"), icon: Grid, color: "bg-slate-50 text-slate-600" },
  ];
      ...
      <section className="container-page !py-0">
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-4 md:gap-6">
          {CATEGORIES.map((cat) => (
            ...
            >
              <div className={`p-4 rounded-2xl transition-all duration-300 ${cat.color} group-hover:scale-110 group-hover:rotate-6`}>
                <cat.icon size={32} strokeWidth={1.5} />
              </div>
              <span className="text-xs md:text-sm font-extrabold text-primary tracking-tight">{cat.label}</span>
            </Link>
          ))}
        </div>
      </section>
